...
from utils import delete_pictures_from_folder, save_line_to_json, read_txt_file, save_line_txt_file
import json
import inspect
import datetime
...
def save_messages(channel, date_obj, f_messages, f_media, media_folder, client, keywords):
    ...
    for message in messages:
        if message['_'] == 'Message':
            ...
            if not any(keyword.lower() in message['message'].lower() for keyword in keywords):
                continue
            ...
            # dict ----------------------#
            print('Description:', message['message'])
            print("Media codec:", message['media'])
                        
            if 'media' in message:
                if message['media'] is not None and message['media'].get('video', '-') == True:
...
if __name__ == "__main__":
	...
	keywords = [
		'Multicopter', 'мультикоптерних', 'БПЛА',
		'Wing', 'XWing', 'X-подібний', 'X-подібні',
		'дрон', 'дрони',
		'Multirotor', 'Квадрокоптер', 'Hexacopter', 'Octocopter',
		'UAV', 'Aerial drone', 'FPV drone', 'Camera drone', 'Flying drone', 'Air drone',
		'Беспилотник', 'Беспилотный летательный аппарат', 'Беспилотник с камерой',
		'Дрон с камерой', 'Дроны для съемки', 'Дрон для аэрофотосъемки',
		'Мультикоптер', 'БПЛА (Беспилотный летательный аппарат)', 'БПЛА с камерой',
		'Гексакоптер', 'Окта́коптер', 'Вертолетный дрон',
		'Дрон з камерою', 'Безпілотник', 'Квадрокоптер', 'Гексакоптер',
		'Окта́коптер', 'Аерофотозйомка', 'Аерозйомка',
		'Drone', 'Quadcopter', 'Hexacopter', 'Octocopter', 'FPV', 'Drone footage', 'Drone camera',
		'Aerial photography', 'Aerial videography', 'Drones with cameras', 'Flying camera',
		'Aerial filming', 'Surveillance drone', 'Inspection drone', 'Recreational drone', 'Professional drone',
		'RC drone', 'Radio-controlled drone', 'Drones for mapping', 'Drones for inspection', 'Drones for surveying',
		'Drones for delivery', 'Agricultural drone', 'Delivery drone'
	]
	...
	for ch in channels:
		...
		if channel is not None:
			...
			if id is not None:
				...
				msg_objects = save_messages(channel, yesterday, f_messages, f_media, media_folder, client, keywords)
	...
	for obj in all_messages_objects: 
		...
		delete_pictures_from_folder(media_folder)
